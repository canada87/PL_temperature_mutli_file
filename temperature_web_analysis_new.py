...
uploadfile_exp = st.file_uploader('Load zip file here', 'zip')
# st.write('It requires "_compare" file generated by "single"')
names = []
if uploadfile_exp:
    zf = zipfile.ZipFile(uploadfile_exp)
    files_exp = dict()
    for i, name in enumerate(zf.namelist()):
        names.append(name)
        files_exp[i] = pd.read_csv(zf.open(name))

type_of_analysis = ['home', 'single', 'learning on single', 'loop numerators', 'empty1', 'empty2', 'empty3', 'empty4', 'empty5', 'Global analysis']
side_selection = st.sidebar.radio('Select the type of analysis', [type_of_analysis[0], type_of_analysis[1], type_of_analysis[2], type_of_analysis[3], type_of_analysis[9]])

if side_selection != type_of_analysis[5] and side_selection != type_of_analysis[9]:
    scelta2 = st.sidebar.radio('type of parameters',('standard scale', 'log scale', 'direct log'))
    log_name = ''
...
for exp_num in range(len(names)):
    PL_mane = names[exp_num].replace('.txt', '')
    ...
    # ██      ██   ██ ██   ██ ██   ██ ██  ██  ██ ██         ██    ██      ██   ██      ██
    # ██      ██   ██ ██   ██ ██   ██ ██      ██ ███████    ██    ███████ ██   ██ ███████
    num_of_scan = int(vet_param[2])
    numb_of_spect = int(vet_param[1])
    bkg_position = 'min' if vet_param[3] == 0 else 'max'
    x_bkg = int(vet_param[4])
    y_bkg = int(vet_param[5])
    remove_cosmic_ray = 'yes' if vet_param[6] == 0 else 'no'
    soglia_derivata = int(vet_param[7])
    wave_inf = float(vet_param[8])
    wave_sup = float(vet_param[9])
    num_bin = int(vet_param[10])
    raggio = float(vet_param[11])
    laser_power = float(vet_param[12])
    AS_min = float(vet_param[13])
    AS_max = float(vet_param[14])
    S_min = float(vet_param[15])
    S_max = float(vet_param[16])
    riga_y_start = int(vet_param[17])
    riga_y_stop = int(vet_param[18])
    pixel_0_x = int(vet_param[19])
    pixel_0_y = int(vet_param[20])
    laser_type = int(vet_param[21])
    lato_cella = float(vet_param[22])
    temp_max = float(vet_param[23])
    cut_min = float(vet_param[24])
    cut_max = float(vet_param[25])
    punti_cross_section_geometrica = vet_param[26] #removed from the analisys but still here in case of future use
    material = vet_param[32]
    num_loop1 = int(vet_param[27])
    num_loop2 = int(vet_param[28])
    soglia_r2 = float(vet_param[29])
    selected_scan = int(vet_param[30])
    T_RT = float(vet_param[31])
    ...
    if uploadfile_exp:
        if side_selection != type_of_analysis[4] and side_selection != type_of_analysis[0] and side_selection != type_of_analysis[7] and side_selection != type_of_analysis[6] and side_selection != type_of_analysis[8]:
            if uploadfile_exp:
                raw_data = data_storage(files_exp[exp_num]).load_map_multiple_files(num_of_scan)
